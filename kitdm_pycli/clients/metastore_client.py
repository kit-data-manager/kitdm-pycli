...
def parse_arguments(args):
    parser = argparse.ArgumentParser(description='Command line client interface for the MetaStore service.')
    ...
    create_schema_parser = operation_subparser.add_parser('createSchema', help='Create a new metadata schema.')
    ...
    create_document_parser = operation_subparser.add_parser('createDocument', help='Create a new metadata document.')
    add_metadata_argument(create_document_parser)
    add_payload_argument(create_document_parser)

    # getSchema [-id 123] [-v 2]
    get_schema_parser = operation_subparser.add_parser('getSchema', help='List metadata for single or multiple '
                                                                         'registered schemas.')
    ...
    get_schemas_parser = operation_subparser.add_parser('getSchemas', help='List metadata for multiple '
                                                                           'registered schemas.')
    add_range_filter_arguments(get_schemas_parser)
    add_pagination_arguments(get_schemas_parser)

    # getDocument [-id 123] [-v 2]
    get_document_parser = operation_subparser.add_parser('getDocument', help='List metadata for single or multiple '
    ...
    get_documents_parser = operation_subparser.add_parser('getDocuments', help='List metadata for multiple '
                                                                               'registered documents.')
    get_documents_parser.add_argument('-sid', '--schemaIds', type=str, nargs='+',
    ...
    get_documents_parser.add_argument('-rr', '--relatedResources', type=str, nargs='+',
    ...
    download_schema_parser = operation_subparser.add_parser('downloadSchema', help='Download a metadata schema.')
    ...
    download_document_parser = operation_subparser.add_parser('downloadDocument', help='Download a metadata document.')
    ...
    update_schema_parser = operation_subparser.add_parser('updateSchema', help='Update a metadata schema.')
    ...
    update_document_parser = operation_subparser.add_parser('updateDocument', help='Update a metadata document.')
    ...
    delete_schema_parser = operation_subparser.add_parser('deleteSchema', help='Delete one or more schemas.')
    add_multiple_identifier_argument(delete_schema_parser)
    delete_schema_parser.add_argument('-s', '--soft', action=argparse.BooleanOptionalAction, default=True,
    ...
    delete_document_parser = operation_subparser.add_parser('deleteDocument', help='Delete one or more documents.')
    add_multiple_identifier_argument(delete_document_parser)
    delete_document_parser.add_argument('-s', '--soft', action=argparse.BooleanOptionalAction, default=True,
...
def main():
    ...
    if args.operation == "createSchema":
        # createSchema -m schema_record.json -pl schema.json
        response = service_client.create(args.metadata, args.payload, "schema", args.auth)
        ...
        response = service_client.create(args.metadata, args.payload, "document", args.auth)
        response = service_client.render_response(response, args.render_as)
        ...
        if len(args.identifier) > 1:
            ...
            response = service_client.get(args.identifier[0], "schema", query_params, args.auth)
        ...
        if len(args.identifier) > 1:
            ...
            query_params = [get_query_param_entry("version", str(args.version))]
            response = service_client.get(args.identifier[0], "document", query_params, args.auth)
        ...
        if args.relatedResources:
            query_params.append(get_query_param_entry("resourceId", ','.join(args.relatedResources)))

        if args.schemaIds:
            query_params.append(get_query_param_entry("schemaId", ','.join(args.schemaIds)))
        ...
        response = service_client.download(args.identifier, "schema", args.version, args.auth)
    elif args.operation == "downloadDocument":
        # downloadDocument -id 123 [-v 1]
        response = service_client.download(args.identifier, "document", args.version, args.auth)
    elif args.operation == "updateSchema":
        # updateSchema -id 123 -m schema_record.json -pl schema.json
        response = service_client.update(args.identifier, args.metadata, args.payload, "schema", args.auth)
        response = service_client.render_response(response, args.render_as)
    elif args.operation == "updateDocument":
        # updateDocument -id 123 -m metadata_record.json -pl metadata.json
        response = service_client.update(args.identifier, args.metadata, args.payload, "document", args.auth)
        response = service_client.render_response(response, args.render_as)
        ...
        if len(args.identifier) > 1:
            ...
            all_results = []
            for identifier in args.identifier:
                response = service_client.delete(identifier, "schema", args.soft, args.auth)
                if not response:
                    service_client.print_error("Schema " + identifier + " could not be deleted.")
        else:
            response = service_client.delete(args.identifier[0], "schema", args.soft, args.auth)
            if not response:
                service_client.print_error("Schema " + args.identifier[0] + " could not be deleted.")
        ...
        if len(args.identifier) > 1:
            ...
            all_results = []
            for identifier in args.identifier:
                response = service_client.delete(identifier, "document", args.soft, args.auth)
                if not response:
                    service_client.print_error("Document " + identifier + " could not be deleted.")
        else:
            response = service_client.delete(args.identifier[0], "document", args.soft, args.auth)
            if not response:
                service_client.print_error("Document " + args.identifier[0] + " could not be deleted.")
