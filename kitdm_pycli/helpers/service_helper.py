...
from keycloak import KeycloakOpenID, KeycloakConnectionError, KeycloakAuthenticationError
...
class ServiceClient(ABC):
    ...
    def __init__(self, debug):
        ...
        try:
            ...
            ServiceClient.print_error("PyCli properties not found at " + properties_filename + ".")
            exit(2)
        except JSONDecodeError as f:
            ServiceClient.print_error("Invalid PyCli properties file format in file " +
                                      properties_filename +
                                      ". Message: " +
                                      f.msg)
            exit(2)
    ...
    @abstractmethod
    def create(self, identifier: str, metadata: str, content: Optional[str], path: Optional[str], auth: bool = False):
        pass

    @abstractmethod
    def update(self, resource_id: str, metadata: Optional[str], content: Optional[str], path: Optional[str], auth: bool = False):
        pass

    @abstractmethod
    def patch(self, resource_id: str, metadata: Optional[str], path: Optional[str], auth: bool = False):
        pass

    @abstractmethod
    def get(self, resource_id: Optional[str], path: str, query_params: Optional[dict], auth: bool = False) -> list:
        pass

    @abstractmethod
    def delete(self, resource_id: Optional[str], path: Optional[str], soft: bool = True, auth: bool = False):
        pass

    @abstractmethod
    def download(self, resource_id: str, path: Optional[str], version: Optional[int],
                 download_path: Optional[str], auth: bool = False):
        pass
    ...
    def login(self, do_auth, headers) -> bool:
        ...
        keycloak_openid = KeycloakOpenID(server_url=self.properties['keycloak']['server_url'],
        ...
        try:
            ...
            if self.token_expires and in_thirty_seconds > self.token_expires and \
                    self.refresh_token and \
                    self.refresh_token_expires and datetime.datetime.now() < self.refresh_token_expires:
                ...
                self.print_debug("No refresh token found. Performing initial login to KeyCloak.")
            ...
            if 'access_token' in token:
                ...
                self.token_expires = datetime.datetime.now() + datetime.timedelta(seconds=token['expires_in'])
                self.refresh_token_expires = datetime.datetime.now() + datetime.timedelta(seconds=token['refresh_expires_in'])
                ...
                if self.debug:
                    ...
                    self.print_debug("Successfully logged in as username: " + userinfo['preferred_username'] +
                                     ", email: " + userinfo['email'] +
                                     ", groups " + str(userinfo['groups']) + ".")
                return True
            else:
                self.print_error("No access_token found in KeyCloak response.")
        except KeycloakConnectionError as e:
            self.print_error("Failed to connect to KeyCloak instance. Message: " + str(e.error_message))
        except KeycloakAuthenticationError as f:
            self.print_error("Failed to authenticate at KeyCloak instance. Message: " + str(f.error_message))
    ...
    def do_get(self, base_url: str, path: str, headers):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_error("Server returned status " +
    ...
    def do_post(self, base_url: str, path: str, headers, payload: str):
        url = base_url + path
    ...
    def do_put(self, base_url: str, path: str, headers, payload: str):
        url = base_url + path
