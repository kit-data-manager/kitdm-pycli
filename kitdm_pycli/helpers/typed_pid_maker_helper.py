...
class TypedPidMakerClient(ServiceClient):
    ...
    def create(self, identifier: str, metadata: str, payload: Optional[str], path: Optional[str], auth: bool = False):
        ...
        # create pid record
        resource_response = self.do_post(self.server_url, endpoint, headers, metadata_content)
    ...
    def update(self, identifier: str, metadata: Optional[str], auth: bool = False):
        ...
        etag = self.do_get_etag(self.server_url, "api/v1/pit/pid/" + identifier, headers)
        headers['If-Match'] = etag
        # do put with metadata_content
        resource_response = self.do_put(self.server_url, "api/v1/pit/pid/" + identifier, headers,
                                        metadata_content)
    ...
    def get(self, identifier: Optional[str], path: Optional[str], query_params: Optional[dict],
            auth: bool = False) -> list:
        """
    ...
    def download(self, resource_id: str, path: Optional[str], version: Optional[int], download_path: Optional[str],
                 auth: bool = False):
        print("Not supported")
